...
//		}); //end of it(2)
		...
		// not acting on it at all
			...
			browser.wait(ExpctCond.elementToBeClickable(zipField),15000);
		...
		it('it strt 3: should evaluate the single coverage selection', function(){
			...
			browser.wait(ExpctCond.elementToBeClickable(howManyWillBeCovered),15000);
...
//////////			    
		...
		// 6/2/17 aAll broker testing removed.  Broker has been non functional ( hcentive ?? ) for some time	
...
//			}
			console.log('it pers6A::: Broker features currently skipped');
		});// end it pers 6A
...
//			RadBtnBrokerYes.click();// used to loose focus from input field
			console.log('it pers6B::: Broker features currently skipped');
		});// end of it Pers6B
...
//			browser.wait(ExpctCond.textToBePresentInElementValue(elementBrokerName ,protVarBrokerName),15000);
//			
...
//			expect(elementBrokerName.getAttribute('value')).toEqual(protVarBrokerName);
			console.log('it pers6C::: Broker features currently skipped')
		});
...
//
	...
	describe('Facilities Page, expect-wait, Select(), Submit()', function(){	
		...
		it('it facs 2: use expected condition to let the page stabilize Test 2', function() {
			browser.wait(ExpctCond.elementToBeClickable($('#search-facilities')),15000);
		...
		it('it facs 3: should find a radio button and select it', function(){
			...
			browser.wait(ExpctCond.visibilityOf(facilitiesToSelectBtn),15000)
...
// expect > --- ?			
	...
	describe('Payment Information page', function(){
		...
		it('it Pmnt1A: Use Expected Condtion to leat page stabilize3', function() {
			browser.wait(ExpctCond.visibilityOf($('#cardName')),15000); // ie: element(by.id('cardName')
		...
		it('it Pmnt1B: Use Expected Condtion to leat page stabilize3', function() {
			browser.wait(ExpctCond.visibilityOf($('#nextButton')),15000);// ie: element(by.id('nextButton')
...
//			var authChkBox	= element(by.id('auth'));
			...
			browser.wait(ExpctCond.elementToBeClickable(authChkBox),15000);
...
//			expect(element(by.id('nextButton')).getAttribute('value')).toBe('Loading');
		...
		//	expect(pmntNextButton.getAttribute('value')).toBe('Loading...'); 
			...
			browser.wait(ExpctCond.textToBePresentInElementValue(pmntNextButton, nextButtonTextPostClick), 15000);
