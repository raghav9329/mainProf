            ...
            if (product.indexOf(pdescription) != -1) {
                ...
                describe('1406- Payment CTA Back Next: ||State:' + sdescription + '||Product:' + pdescription + '||', function() {
                    ...
                    it('Step-1: Validate and Verify the Errors of both the Client and Server in the Payment Page', function() {
                        ...
                        expect(payment.getCCValidationMessages()).toEqual(TestData.paymentErrors, 'Verify the Credit Card "Client Validation" Error Messages is same as: ' +TestData.paymentErrors)
                        expect(payment.getBillingAddressValidationMessages()).toEqual(TestData.paymentAddressErrors, 'Verify the Payment Address "Client Validation" Errors are same as: ' +TestData.paymentAddressErrors);
                        expect(payment.getCCServerValidationMessages()).toEqual(TestData.paymentErrors, 'Verify the Credit Card "Server Validation" Error Messages is same as: ' +TestData.paymentErrors);
                        expect(payment.getBillingAddressServerValidationMessages()).toEqual(TestData.paymentAddressErrors, 'Verify the Payment Address "Server Validation" Errors are same as: ' +TestData.paymentAddressErrors);
                    });
                    ...
                    it('Step-3:Should be navigate to receipt page', function() {
                        ...
                        expect(payment.errAuth.getText()).toEqual(TestData.errorMsgAuth, 'Verify the Authenticate Check box "Client Validation" Error Message is same as: ' +TestData.errorMsgAuth);
                        expect(payment.serErrAuth.getText()).toEqual(TestData.errorMsgAuth, 'Verify the Authenticate Check box "Server Validation" Error Message is same as: ' +TestData.errorMsgAuth);
                        payment.authChkBox.check();
                        if (pdescription == 'AHMO' || pdescription == 'APPO') {
                            payment.frequencyAnnualy.select();
                        }
                    ...
                    it('Step-4:Should be navigate to receipt page  ', function() {
                        payment.fillpayment(TestData, true, true);
                        ...
                        expect(payment.errCardNumber.getText()).toEqual(TestData.errorMsgCard, 'Verify the Card Number "Client Validation" Error Messages is sa,e as: ' +TestData.errorMsgCard);
                        expect(payment.serErrCardNumber.getText()).toEqual(TestData.errorMsgCard, 'Verify the Card Number "Server Validation" Error Messages is sa,e as: ' +TestData.errorMsgCard);
