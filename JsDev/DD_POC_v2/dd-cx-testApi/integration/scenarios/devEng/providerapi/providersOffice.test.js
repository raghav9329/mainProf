...
var TestData = require('../../../testData/devEng/providerapi/providersOffice.json');
...
describe('Providers', function() {
    ...
    dataProvider(TestData.customerNetworktestdata, function(data, description) {
        it(" Validating CustomProviderNetworks for the given officeId" + Utility.getapiurl('LOCATIONS', data.practiceLocationNumber, data.params) + " ", function(doneFn) {
            //Construction of REST API URL HOST+Resource +Params
            let apiurl = Utility.getapiurl('LOCATIONS', data.practiceLocationNumber, data.params);
            //REST API call using Frisby Node Module & Verifying Status code and Json data set using Frisby expect.
            ...
            frisby.get(apiurl)
                .then(function(res) {
                    var json = res.json; // converting resp to JSON format
                    expect(res.status).toEqual(200, "Verify that satatus code should be 200");
                    expect(typeof json.practiceLocationNumber).toEqual('string', "Verify that 'practiceLocationNumber' data type should be 'string'");
                    expect(json.practiceLocationNumber).toEqual(data.practiceLocationNumber, "Verify that 'practiceLocationNumber' should be " + data.practiceLocationNumber);
