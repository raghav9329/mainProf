'use strict';
var TestData = require('../../../testData/devEng/providerapi/providers.json');
...
describe('Provider Directory Search Service retrieve the provider details by Provider API : ', function() {
    ...
    it(" Given practiceLocationNPI number in free_text and zipcode" + providerdata.test22.params.zipcode + 
        "freeText" + providerdata.test22.params.free_text + "  Then validating the response with freetext value w.r.t  npi   " + Utility.getapiurl('PROVIDERS', '', providerdata.test22.params) + " ",
        function(doneFn) {

            let apiurl = Utility.getapiurl('PROVIDERS', '', providerdata.test22.params);
            let data = providerdata.test22;
            console.log("api url -------------" + apiurl);
            ...
            frisby.get(apiurl)
                .then(function(res) {
                    ...
                    json.providers.forEach(function(provider) {
                        ...
                        let apiurl1 = Utility.getapiurl('LOCATIONS', provider.practiceLocationNumber, '');
                        console.log(apiurl1)
                        return frisby.get(apiurl1)
                            .then(function(providerres) {
                                let response = providerres.json;
                                expect(providerres.status).toEqual(data.verify.status, "verify that  status code should be " + data.verify.status);
                                expect(response.practiceLocationNpi).toEqual(data.verify.free_text, "Verify that free_text value equal to " + data.verify.free_text);
                            }).done(doneFn);
    ...
    it(" Given Language in free_text and zipcode" + providerdata.test23.params.zipcode + 
        "freeText" + providerdata.test23.params.free_text + "  Then validating the response with freetext value w.r.t  npi   " + Utility.getapiurl('PROVIDERS', '', providerdata.test23.params) + " ",
        function(doneFn) {

            let apiurl = Utility.getapiurl('PROVIDERS', '', providerdata.test23.params);
            let data = providerdata.test23;
            console.log("api url -------------***" + apiurl);
                    ...
                    json.providers.forEach(function(provider) {
                        ...
                        return frisby.get(apiurl1)
                            .then(function(providerres) {
                                let response = providerres.json;
                                 expect(providerres.status).toEqual(data.verify.status, "verify that  status code should be " + data.verify.status);
                                    if(response.providerLanguages.length > 0){ 
                                        if( response.providerlanguage.indexOf(data.verify.free_text) != -1){
                                        expect(response.providerLanguages).toContain(data.verify.free_text, "Verify that free_text value equal to " + data.verify.free_text);     
                                    }
                                    }else{
                                        expect(response.practiceLocationLanguages).toContain(data.verify.free_text, "Verify that free_text value equal to " + data.verify.free_text);     
                                    }
    ...
    it(" Given lat = " + providerdata.test24.params.lat + " long =" + providerdata.test24.params.long +
        "  sourceNetwork = " + providerdata.test24.params.sourceNetwork + " targetNetwork = " + providerdata.test24.params.targetNetwork + "  Then validating the response with custom providerNetworks  " + Utility.getapiurl('PROVIDERS', '', providerdata.test24.params) + " ",
        function(doneFn) {

            let apiurl = Utility.getapiurl('PROVIDERS', '', providerdata.test24.params);
            let data = providerdata.test24;
            console.log("api url -------------" + apiurl);

            frisby.get(apiurl)
                .then(function(res) {
                    let json = res.json;
                    expect(res.status).toEqual(data.verify.status, "verify that  status code should be 200 ");
                    ...
                    json.providers.forEach(function(provider) {
                        ...
                        expect(networks).toContain(data.verify.network, "Verify that newtworks contain custom network" + data.verify.network);
                    })
