    ...
    dataProvider(TestData.Personalinfo.DOB, function(data, description) {
        if (data.ExecutionFlag) {
            it('Validate DOB field with value "' + data.DOB + '"', function() {
                perInfo.fieldBdMM.setText(browserActions.getDatePart(data.DOB, 'month'));
                perInfo.fieldBdDD.setText(browserActions.getDatePart(data.DOB, 'Date'));
                perInfo.fieldBdYyyy.setText(browserActions.getDatePart(data.DOB, 'Year'));
                perInfo.Next.click();
                browser.sleep(2000);
                expect(perInfo.errBirthDate.getText()).toEqual(data.ErrorMsg_BirtDate);
            });
        };
    });
