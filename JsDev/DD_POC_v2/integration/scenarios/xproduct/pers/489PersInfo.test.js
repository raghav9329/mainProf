        ...
        dataProvider(sData.products, function(tData, pdescription) {
            if (product.indexOf(pdescription) != -1) {
                ...
                describe('489: AddrHome flds-PersInfo State: ||State:' + sdescription + '||Product:' + pdescription + '||', function() {
                    ...
                    // Validate navigation for a zip code change to an unsupported state
                    // Unsupported state address is set to Arizona (AZ) for now. Will need to change this when we start supporting Arizona
                    it('21: Should Enter unsupported state address and validate redirection to hCentive Plan Options Page', function() {
                        Utility.openApplication('', tData.product);
                        enrollPage.enterHomePageDetails(tData.enrollData);
                        var data = TestData.Address_Unsupported;
                        perInfo.fieldHomeAddr.setText(data.Home);
                        perInfo.fieldCity.setText(data.City);
                        perInfo.fieldState.setText(data.State);
                        perInfo.fieldZipCode.setText(data.ZIPcode);
                    ...
                    // Shounak: Added to validate story # CXINIT-3677
                    // Validate navigation for a zip code change to a supported state
                    it('22: Should Enter supported state address and validate redirection to CX Plan Options Page', function() {
                        Utility.openApplication('', tData.product);
                        enrollPage.enterHomePageDetails(tData.enrollData);
                        var data = TestData.Address_Supported;
                        perInfo.fieldHomeAddr.setText(data.Home);
                        perInfo.fieldCity.setText(data.City);
                        perInfo.fieldState.setText(data.State);
                    ...
                    // Shounak: Added to validate story # CXINIT-3677
                    // Validate navigation from Back To Quotes to CX Plan Details Page
                    // 01.24.2018: This will fail currently (for DIRECT), as some part of this redirection is a part of next sprint's work
                    it('23: Pressing BackToQuote CTA should take user to CX Plan Details Page', function() {
                        Utility.openApplication('', tData.product);
                        enrollPage.enterHomePageDetails(tData.enrollData);
