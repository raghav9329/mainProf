...
/********************************************************
 *  Create a list node: by def this represents a list. Capture a pointer to the 
 *  beginning of the list in testList. then use myList and be able to reset
 *  back to the beginning any time with myList = testList
 */
// ToDo Should make testList as named, "frontOfList, or beginOfList, etc.
...
* Remember if this([late:] hummmmmm do I mean the code directly above ? ) 
* (([Later(immediately)]: I must have not noticed the entire block of code
* and this text all within the same comment block))
* code is uncommented and puts the first
 ...
 */
		...
		System.out.print("Let's print out the List:: ");
		myList.print();
		...
		System.out.println();
		System.out.print("Now let's set some duplicates:: ");
...
//		System.out.println("Number of duplicates is "	+ myList.countDupsTogether(myList));	
		...
		System.out.println();
		System.out.print("creating Solution Variable... :");	
		
		System.out.print("Calling solution on testList...");
		myList = testList;
...
//		reverseList.reverseTheList(myList);
		...
		int getTheSize = 0;
		getTheSize = myList.size();
		System.out.println("new call to size is :" +getTheSize);

		getTheSize = myList.count(myList);
		System.out.println("new call to count is:" +getTheSize);
